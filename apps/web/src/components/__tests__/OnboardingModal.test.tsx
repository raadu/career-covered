...
describe('OnboardingModal', () => {
    ...
    it('renders correctly when open', () => {
        renderWithProviders(<OnboardingModal isOpen={true} onComplete={onComplete} />);
        expect(screen.getByText(/It’s so easy to start!/i)).toBeInTheDocument();
        expect(screen.getByPlaceholderText(/Enter your Groq API Key here/i)).toBeInTheDocument();
    });

    it('shows help title when onClose is provided', () => {
        renderWithProviders(<OnboardingModal isOpen={true} onComplete={onComplete} onClose={vi.fn()} />);
    ...
    it('dispatches setApiKey and calls onComplete when Start is clicked', () => {
        const { store } = renderWithProviders(<OnboardingModal isOpen={true} onComplete={onComplete} />);
    ...
    it('calls onClose when close icon is clicked', () => {
        const onClose = vi.fn();
        renderWithProviders(<OnboardingModal isOpen={true} onComplete={onComplete} onClose={onClose} />);
