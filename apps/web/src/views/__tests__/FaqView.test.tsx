...
describe('FaqView', () => {
  ...
  it('renders all 8 FAQ questions', () => {
    renderWithProviders(<FaqView />);
    expect(screen.getByText(/How do I log in or sign up using Groq/i)).toBeInTheDocument();
    expect(screen.getByText(/Will I be charged with my credit card/i)).toBeInTheDocument();
    expect(screen.getByText(/Is my data secure/i)).toBeInTheDocument();
    expect(screen.getByText(/How many cover letters can I create for free/i)).toBeInTheDocument();
    expect(screen.getByText(/What is a Groq API key/i)).toBeInTheDocument();
    expect(screen.getByText(/Can I customize the tone and style/i)).toBeInTheDocument();
    expect(screen.getByText(/How do I export my cover letter/i)).toBeInTheDocument();
  ...
  it('has remaining 3 questions collapsed by default', () => {
    renderWithProviders(<FaqView />);
    const buttons = screen.getAllByRole('button');
    const collapsed = buttons.filter((b) => b.getAttribute('aria-expanded') === 'false');
  ...
  it('toggles a question open when clicked', () => {
    ...
    const customizeBtn = screen.getByText(/Can I customize the tone and style/i);
    expect(customizeBtn.closest('button')?.getAttribute('aria-expanded')).toBe('false');

    fireEvent.click(customizeBtn);

    expect(customizeBtn.closest('button')?.getAttribute('aria-expanded')).toBe('true');
    // Answer should be visible
    expect(screen.getByText(/You have full control over the tone/i)).toBeInTheDocument();
  });
  ...
  it('toggles a question closed when clicked', () => {
    ...
    expect(firstBtn.closest('button')?.getAttribute('aria-expanded')).toBe('true');

    fireEvent.click(firstBtn);

    expect(firstBtn.closest('button')?.getAttribute('aria-expanded')).toBe('false');
  });
  ...
  it('contains FAQPage schema markup', () => {
    ...
    const faqPageDiv = container.querySelector('[itemType="https://schema.org/FAQPage"]');
