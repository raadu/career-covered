...
const DarkModeToggle = ({ isExpanded }: DarkModeToggleProps) => {
  ...
  return (
    ...
    >
      {isDark ? (
        <FaSun size={18} className="lg:size-5 group-hover:scale-110 group-hover:text-amber-500 transition-all shrink-0" />
      ) : (
        <FaMoon size={16} className="lg:size-[18px] group-hover:scale-110 group-hover:text-blue-500 transition-all shrink-0" />
      )}
      {isExpanded && (
