...
describe('CustomizeModal', () => {
    ...
    it('renders nothing when isOpen is false', () => {
        const { container } = render(
            <CustomizeModal isOpen={false} onClose={onClose} initialOptions={initialOptions} onSave={onSave} hasTemplate={true} />
        );
    ...
    it('calls onSave with correct data when Save is clicked', () => {
        ...
        expect(onSave).toHaveBeenCalledWith({ options: initialOptions, customPrompt: '' });
    });
    ...
    it('shows validation error and blocks save when limitWords is enabled with invalid count', () => {
        ...
        const input = screen.getByPlaceholderText(/Numbers should be between 50 - 1000/i);
        fireEvent.change(input, { target: { value: '25' } });
        fireEvent.click(screen.getByRole('button', { name: /Save Options/i }));

        expect(screen.getByText(/Numbers should be between 50 - 1000/i)).toBeInTheDocument();
    ...
    it('handles malicious custom prompt safely (passes raw value to parent, sanitized at logic layer)', () => {
        ...
        const textarea = screen.getByPlaceholderText(/Wanna add or remove anything/i);
